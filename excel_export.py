...
Формат:
- Одна книга: outputs/operations.xlsx
- Для каждого чата отдельный лист (название группы).
- На листе несколько таблиц, расположенных по ГОРИЗОНТАЛИ.
- Таблицы ВЫРОВНЕНЫ ПО ДАТАМ:
  если в один день были "Поступление", "Оплата ПП", "Конвертация" и т.д. —
  они будут находиться на ОДНИХ И ТЕХ ЖЕ строках (с пустыми промежутками там, где нет операций).
- Верхние две строки зафиксированы (freeze_panes = 'A3').
- Внизу листа — итоги по валютам.

Изменения:
- Убрана сетка (границы не рисуются).
- Описание больше НЕ колонкой: для ВСЕХ таблиц описание переносится в комментарий к "ключевой сумме".
- Для "Конвертация" убрана колонка "Банк" (используем description как комментарий к "Сумма платежа").
- Строки не растягиваем: wrap_text=False, высота строк фиксирована.
"""
...
from config import CURRENCIES, OPERATION_TYPES
...
def _build_payment_maps(operations_sorted: List[Tuple]) -> tuple:
    """
    Строим два словаря:
    1) payment_op_id -> сумма SWIFT (USD, >0)
    2) payment_op_id -> сумма Комиссии 1% (в валюте операции, >0)

    Ищем следующие 2 операции после 'Оплата ПП' и собираем их в карту.
    """
...
def _group_rows_by_date(rows: List[List[object]], date_col_index: int = 1) -> dict:
    """
    rows: список строк таблицы, где в колонке date_col_index лежит "dd.mm.yyyy"
    returns: { "dd.mm.yyyy": [row1, row2, ...] }
    """
    m = defaultdict(list)
    for r in rows:
...
def _build_date_grid(rows_by_type: Dict[str, dict]) -> tuple[list[str], dict[str, int]]:
    """
    rows_by_type: {op_type: {date_str: [rows...]}}
    returns:
      all_dates_sorted: ["01.12.2025", ...]
      span_by_date: {"01.12.2025": 3, ...}  # max строк среди всех типов на эту дату
    """
    all_dates = set()
...
def _build_comment_map_for_type(ops_of_type: List[Tuple]) -> Dict[Tuple[str, int], str]:
    """
    Комментарии по типу операции.
    Ключ: (date_str, idx) — порядковый номер внутри даты (0..)
    Значение: description (если есть)
    """
...
def _set_comment(cell, text: str):
    """Безопасно ставим комментарий (не ставим на пустые)."""
    if not text:
        return
    txt = text.strip()
    if not txt:
...
def _write_operations_tables_for_chat(ws, operations: list, styles: dict, chat_id: int, db: Database):
    ...
    for op_type in ordered_types:
        ...
        if op_type == "Конвертация":
            ...
            # ✅ все остальные — без "Описание" колонкой
    ...
    for table in prepared_tables:
        ...
        for d in all_dates_sorted:
            ...
            for k in range(span):
                ...
                # фикс высоты строк, чтобы ничего не тянуло
                ws.row_dimensions[excel_row].height = 18

                # описание (comment) для этой строки (если есть)
...
def export_to_excel(
    ...
    for chat_id, chat_name, chat_type, *_ in chats:
        ...
        # Пропускаем чаты без операций в выбранный период
        if not operations:
    ...
    # Если ни одного листа не создано
    if sheets_created == 0:
...
def export_to_excel_single_chat(
    ...
    # ✅ ИСПРАВЛЕНО: получаем операции и передаём правильные параметры
    operations = db.get_operations(chat_id, limit=100000)
    _write_operations_tables_for_chat(ws, operations, styles, chat_id, db)
